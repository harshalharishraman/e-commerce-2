    ...
    try {
        const result = await knex.transaction(async (trx) => {
            return trx.raw(`
                ...
                item_totals AS (
                    SELECT
                        cart_items_tb.product_id,
                        SUM(cart_items_tb.quantity) AS quantity
                    FROM cart_items_tb
                    INNER JOIN expired_carts
                        ON expired_carts.id = cart_items_tb.cart_id
